...
export async function POST(req: NextRequest) {
  try {
    ...
    const newConsultation: ConsultationCreate = {
      ...
      status: 'unread', // 初期ステータス
    };

    // DB に登録
    const sql = `
      INSERT INTO consultations
      (target_type, place_type, content_type, suggestion_topic, needs_reply, email, message, encrypted_message, token_hash, ip_hash, user_agent, referer_url, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const args = [
    ...
    // メール通知（needs_reply が true の場合のみ）
    if (newConsultation.needs_reply && newConsultation.email) {
      const adminEmail = process.env.ADMIN_NOTIFICATION_EMAIL;
      if (adminEmail) {
        await sendNotificationEmail(
          adminEmail,
          `新しい相談が届きました: ${newConsultation.suggestion_topic}`,
          `<p>テーマ: ${newConsultation.suggestion_topic}</p>
           <p>本文: ${newConsultation.message}</p>`
        );
      }
    }
