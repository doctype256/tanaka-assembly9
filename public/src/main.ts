...
interface StaticTexts {
  ...
  hero: {
    ...
    title_line2: string;
    subtitle: string;
    button_consult: string;
    button_credo: string;
  };
  services: {
    sub_title: string;
    main_title_part1: string;
    main_title_part2: string;
    main_title_part3: string;
    items: {
      title: string;
      desc: string;
      link_text: string;
...
async function loadStaticTexts() {
    const data = await fetchJSON<StaticTexts>('static-texts.json');
    if (!data) return;
...
// 2. 解決事例の挿入処理 (動的生成)
// ----------------------------------------------------
async function loadCaseFiles(): Promise<void> {
