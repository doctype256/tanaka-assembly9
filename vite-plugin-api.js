    ...
    return {
        ...
        async configureServer(server) {
            ...
            server.middlewares.use('/uploads', (req, res, next) => {
                var _a;
                const encodedFileName = (_a = req.url) === null || _a === void 0 ? void 0 : _a.replace(/^\//, '');
                if (!encodedFileName) {
                    res.writeHead(404);
                    res.end('Not found');
                    return;
            ...
            server.middlewares.use('/api', async (req, res, next) => {
                ...
                // ===== コメント API =====
                if ((_a = req.url) === null || _a === void 0 ? void 0 : _a.startsWith('/comments')) {
                    // GET: 記事のコメント取得
                    if (req.method === 'GET') {
                ...
                // ===== お問い合わせ API =====
                if ((_b = req.url) === null || _b === void 0 ? void 0 : _b.startsWith('/contacts')) {
                    // GET: お問い合わせ取得
                    if (req.method === 'GET') {
                        const url = new URL(req.url || '', `http://${req.headers.host}`);
                        const password = url.searchParams.get('password');
                ...
                // ===== ご相談ポスト API =====
                if ((_c = req.url) === null || _c === void 0 ? void 0 : _c.startsWith('/posts')) {
                    // GET: ポスト取得
                    if (req.method === 'GET') {
                        const url = new URL(req.url || '', `http://${req.headers.host}`);
                        const password = url.searchParams.get('password');
                ...
                // ===== プロフィール API =====
                if ((_d = req.url) === null || _d === void 0 ? void 0 : _d.startsWith('/profile')) {
                    // GET: プロフィール取得
                    if (req.method === 'GET') {
                        const url = new URL(req.url || '', `http://${req.headers.host}`);
                        const password = url.searchParams.get('password');
                ...
                // ===== キャリア API =====
                if ((_e = req.url) === null || _e === void 0 ? void 0 : _e.startsWith('/career')) {
                    // GET: キャリア一覧取得
                    if (req.method === 'GET') {
                        const url = new URL(req.url || '', `http://${req.headers.host}`);
                        const password = url.searchParams.get('password');
                ...
                // ===== PDF API =====
                if ((_f = req.url) === null || _f === void 0 ? void 0 : _f.startsWith('/pdfs')) {
                    // GET: PDF一覧取得
                    if (req.method === 'GET') {
                        try {
                            const result = await db.execute({
